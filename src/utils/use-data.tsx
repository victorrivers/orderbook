...
export function useData(connectionState: ConnectionState, selectedProduct: Product): {
    snapshotMessage: DataMessage;
    deltaMessage: DataMessage;
} {

    const [subscribedProduct, setSubscribedProduct] = useState<Product>();
    const [snapshotMessage, setSnapshotMessage] = useState<DataMessage>({bids:[], asks:[]});
    const [deltaMessage, setDeltaMessage] = useState<DataMessage>({bids:[], asks:[]});
